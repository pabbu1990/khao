        ...
        <div className="overflow-hidden rounded-xl bg-white shadow-sm">
          <table className="w-full text-sm">
            <thead className="bg-panel text-ink/60">
              <tr><th className="text-left p-3">Customer</th><th className="text-left p-3">Status</th><th className="text-left p-3">Pay</th><th className="text-right p-3">Total</th></tr>
            </thead>
            <tbody>
              {rows.length === 0 && <tr><td colSpan={4} className="p-4 text-center text-ink/40">No orders yet.</td></tr>}
              {rows.map((o) => (
